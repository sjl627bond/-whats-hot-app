(function initialiseApp(windowObject, documentObject) {
  ...
  async function loadVenues() {
    ...
    try {
      ...
      const [hashRoute, hashId] = windowObject.location.hash.slice(1).split('/'); if (!handleDeepLink() && hashRoute === 'venue' && hashId && venueById(hashId)) openVenue(hashId);
  ...
  async function useLocation() {
    if (!windowObject.GoHottMobile.locationRationaleAccepted()) {
      const accepted = windowObject.confirm('Allow location while using GoHott? Location is optional. It improves distances and lets the server assess check-in proximity. Precise coordinates are never shown publicly.');
      if (!accepted) { const explanation = el('#location-notice'); explanation.hidden = false; explanation.textContent = 'Location stays off. City browsing still works normally.'; return null; }
      windowObject.GoHottMobile.acceptLocationRationale();
    }
