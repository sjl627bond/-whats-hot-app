...
['user_data_export_requests_user_recent_idx','push_notification_devices_user_all_idx','client_error_reports_user_idx'].forEach((index) => assert.match(migration, new RegExp(index, 'i'), `${index} must cover account lifecycle lookups`));
...
assert.match(migration, /user_id uuid not null references auth\.users\(id\) on delete cascade/i); assert.match(migration, /user_id uuid references auth\.users\(id\) on delete set null/i);
assert.doesNotMatch(migration, /alter table public\.(profiles|follows|messages|nightlife_plans|social_notifications|user_blocks|live_looks|venues|check_ins)/i);
