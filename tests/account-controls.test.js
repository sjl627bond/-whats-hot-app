...
assert.match(shell, /id="delete-account-modal"[\s\S]+id="delete-account-form"[\s\S]+name="password"[\s\S]+name="confirmed"[\s\S]+type="submit" disabled/);
assert.match(app, /if \(!form\.elements\.confirmed\.checked\)/);
assert.match(app, /GoHottAuth\.reauthenticate\(form\.elements\.password\.value\)/);
assert.match(app, /GoHottAuth\.completeAccountDeletion\(\)/);
...
const client = {
  functions: { invoke: async (name, options) => { functionCalls += 1; assert.equal(name, 'delete-account'); functionBody = options.body; return { data: { deleted: true }, error: null }; } },
...
(async () => {
  ...
  assert.equal(functionCalls, 0, 'an unauthenticated request must be rejected before a server call');
  currentUser = { id: 'user-a' };
  const request = await window.GoHottData.requestAccountDeletion();
  assert.equal(request.deleted, true);
  assert.deepEqual({ ...functionBody }, { confirmation: true });
  assert.equal('user_id' in functionBody, false, 'the client must never choose the deletion target');
